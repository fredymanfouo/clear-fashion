...
console.log(sortedProductsByDate);
...
const filteredProducts = filterByPriceRange(marketplace, 50, 100);
...
const averagePrice = calculateAveragePrice(marketplace);
...
for (const brand in brands) {
  ...
  const p90Index = Math.ceil(0.9 * length) - 1;
  const p90 = products[p90Index].price;
...
function isNewProduct(releaseDate) {
  const twoWeeksAgo = new Date();
  twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
  return releaseDate >= twoWeeksAgo;
}

function logNewProducts(products) {
  const newProducts = products.filter(product => isNewProduct(product.releaseDate));
...
function logReasonablePriceShop(products) {
  const reasonablePrices = products.every(product => isReasonablePrice(product.price));
  if (reasonablePrices) {
...
logReasonablePriceShop(COTELE_PARIS);
...
function deleteProduct(products, uuid) {
...
deleteProduct(COTELE_PARIS, Uuid);
